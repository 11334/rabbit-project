...
import { deleteMemberCartAPI, getMemberCartAPI, putMemberCartSelectedAPI, putMemperCartBySkuIdAPI } from '@/services/cart';
...
const onChangeCount = (e: InputNumberBoxEvent) => {
  // console.log(e);
  putMemperCartBySkuIdAPI(e.index, { count: e.value })
}
...
const onChangeSelected = (item: CartItem) => {
  ...
  // 后端数据更新
  putMemperCartBySkuIdAPI(item.skuId, { selected: item.selected })
}
        ...
        <uni-swipe-action>
          ...
          <uni-swipe-action-item v-for="item in cartList" :key="item.skuId" class="cart-swipe">
            ...
            <view class="goods">
              ...
              <!-- 商品数量 -->
              <view class="count">
                <vk-data-input-number-box v-model="item.count" :min="1" :max="item.stock" @change="onChangeCount"
                  :index="item.skuId" />
